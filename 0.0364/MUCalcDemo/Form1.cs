...
namespace MUCalcDemo
{
    public partial class Form1 : Form
    {
        ...
        private string treatmentPlanName; //nazwa planu
        ...
        //do formatki wynikowej
        private ListViewColumnSorter lvwColumnSorter;

        private ListView beamParamValuesListview;
        ...
        private void mainOperations(Dataset _ds) //glowna funkcja
        {
            ...
            Dataset dsFractionGroupSequence;

            if (_ds != null)
            {
                dsFractionGroupSequence = _ds.GetItem(Tags.FractionGroupSeq, 0);
            }
            else
            {
                dsFractionGroupSequence = null;
            }
            ...
            if (dsFractionGroupSequence != null)
            {
                try
                {
                    this.treatmentPlanName = _ds.GetString(Tags.RTPlanName); //pobranie nazwy planu

                    string message = "Nazwa planu: " + this.treatmentPlanName + Environment.NewLine;
                    ...
                    message += "Liczba wiązek: " + numberOfBeams.ToString() + Environment.NewLine;
                    ...
                    MessageBox.Show("Pomyślnie załadowano plik RT Planu!", "Sukces", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
        ...
        private Form countedMUForm(string _trPlanName, int _beamNumber, double[,] _beamParams, string[] _headers)
        {
            ...
            countedMU.Text = _trPlanName + ": wynik dla " + _beamNumber.ToString() + " wiązek";
            ...
            countedMU.Icon = Properties.Resources.Results;

            #endregion

            #region parametry el formatki

            this.beamParamValuesListview = new ListView();
            this.beamParamValuesListview.Size = new Size(475, 130);
            this.beamParamValuesListview.Location = new Point(5, 5);
            this.beamParamValuesListview.Scrollable = true;
            this.beamParamValuesListview.FullRowSelect = true;
            this.beamParamValuesListview.AllowColumnReorder = true;
            this.beamParamValuesListview.View = View.Details;
            this.beamParamValuesListview.ListViewItemSorter = this.lvwColumnSorter; //przypisanie obiektu klasy sortowania
            this.beamParamValuesListview.ColumnClick += new ColumnClickEventHandler(this.resFormBeamParamListview_ColumnClick);
            ...
            this.beamParamValuesListview.Columns.Add(headers);
            ...
            for (int i = 0; i < _beamNumber; i++)
            {
                ...
                MUValueListviewItem.SubItems.Add(new ListViewItem.ListViewSubItem(MUValueListviewItem, _beamParams[i, 7].ToString()));

                #region kolorowanie roznicy
                if (_beamParams[i, 2] < 3)
                {
                    MUValueListviewItem.BackColor = Color.Green;
                }
                else if (_beamParams[i, 2] >= 3 && _beamParams[i, 2] < 5)
                {
                    MUValueListviewItem.BackColor = Color.Yellow;
                }
                else if (_beamParams[i, 2] >= 5)
                {
                    MUValueListviewItem.BackColor = Color.Red;
                }
                #endregion

                this.beamParamValuesListview.Items.Add(MUValueListviewItem);
            }

            countedMU.Controls.Add(this.beamParamValuesListview);
        ...
        private void customMsgBox(Icon winIcon)
        {
        ...
        private void countMUButton_Click(object sender, EventArgs e)
        {
            ...
            double percentDiff;
            double[,] beamParams = new double[this.numberOfBeams, 8];
            ...
            if (this.properRTPlanLoaded) //jezeli zaladowano prawidlowy plik
            {
                for (int i = 0; i < this.numberOfBeams; i++)
                {
                    ...
                    beamParams[i, 3] = Math.Round(Convert.ToDouble(x.Replace('.', ',')), 2);
                    beamParams[i, 4] = Math.Round(Convert.ToDouble(y.Replace('.', ',')), 2);
                    beamParams[i, 5] = Math.Round(this.SSD[i], 2);
                    beamParams[i, 6] = Math.Round(this.SSDDepth[i], 2);
                    beamParams[i, 7] = Math.Round(this.radDepth[i], 2);
                        ...
                        if (x != "" && y != "") //sprawdzamy kompletnosc danych
                        {
                            ...
                            percentDiff = Math.Round(((Math.Abs(beamParams[i, 0] - beamParams[i, 1])) / beamParams[i, 1]) * 100, 2);
                            beamParams[i, 2] = percentDiff;
                        }
                }

                if (tmpMsg != "") //jezeli cokolwiek policzono
                {
                    Form countedMUForm = this.countedMUForm(this.treatmentPlanName, this.numberOfBeams, beamParams, paramHeaders);
        ...
        {
            MessageBox.Show("MUCalc " + this.progVersion + "\n\nZmiany:\n- nowe ikonki\n- dodano sortowanie danych w kolumnach formatki wynikowej\n- dodano przycisk czyszczenia danych z okna głównego\n\n(c) Łukasz Traczewski 2012-2013", "Info o programie", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        //metoda sortujaca kolumny w przypadku klikniecia
        private void resFormBeamParamListview_ColumnClick(object sender, ColumnClickEventArgs e)
        {
            ...
            this.beamParamValuesListview.Sort();
        }
    }
}
