...
namespace MUCalcDemo
{
    public partial class Form1 : Form
    {
        ...
        public Form1(string _progVersion)
        {
            ...
            ColumnHeader MU = new ColumnHeader();
            MU.Width = 50;
            MU.Text = "MU";

            listView1.Columns.Add(MU);
            listView1.Columns.Add(new ColumnHeader("X"));
            listView1.Columns.Add(new ColumnHeader("Y"));
            listView1.Columns.Add(new ColumnHeader("SSD"));

            listView1.Items.Add(new ListViewItem("Test"));
            listView1.Items.Add(new ListViewItem("Test2"));
            listView1.Items.Add(new ListViewItem("Test3"));
        }
        ...
        private double countSideSize(double[] _leafJawPositions)
        {
            ...
            for (int i = 0; i < numberOfJawsPairs; i++)
            {
                ...
                {
                    jawsAreas[i] = Math.Abs(jawsMinus[i]) + Math.Abs(jawsPlus[i]);
                }
                else
                {
                    if (jawsMinus[i] != 0)
                    {
                        jawsAreas[i] = jawsMinus[i];
                    }
                    else if (jawsPlus[i] != 0)
                    {
                        jawsAreas[i] = jawsPlus[i];
                    }
                    else
                    {
                        jawsAreas[i] = 0;
