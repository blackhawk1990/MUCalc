...
namespace MUCalcDemo
{
    public partial class Form1 : Form
    {
        ...
        private double[] beamRefDoses; //dawki na glebokosci referencyjnej(cGy)
        ...
        private DOSELoad doseFileLoad()
        {
            DOSELoad doseload = new DOSELoad("");

            if (openDoseFileDialog.ShowDialog() == DialogResult.OK)
            {
                if (openDoseFileDialog.FileName != "")
                {
                    doseload = new DOSELoad(openDoseFileDialog.FileName);
                }
            }
        ...
        private void mainOperations(Dataset _ds) //glowna funkcja
        {
            ...
            this.beamRefDoses = new double[this.numberOfBeams];
            ...
            {
                for (int j = 0; j < this.numberOfBeams; j++)
                {
                    this.openDoseFileDialog.Title = j > 0 ? this.openDoseFileDialog.Title.Substring(0, (this.openDoseFileDialog.Title.Length - 2)) + " " + (j + 1) : this.openDoseFileDialog.Title + " " + (j + 1);

                    DOSELoad doseload = this.doseFileLoad();
                    ...
                    if (doseload.success)
                    {
                        ...
                        {
                            //this.beamRefDoses[doseload.beamNumber - 1] = Convert.ToDouble(doseData[1][1].Replace('.', ',')) * 100;
                        ...
                        {
                            MessageBox.Show("Ten plik nie zawiera potrzebnych danych!" + Environment.NewLine + "Spróbuj załadować inny plik", "Błąd", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            this.openDoseFileDialog.Title = "Wybierz plik RT Dose";
            #endregion
            ...
            for (int i = 0; i < this.numberOfBeams; i++)
            {
                ...
                if (!doseLoaded[i] || this.radDepth[i] == 0)
                {
        ...
        private void testbutton_Click(object sender, EventArgs e)
        {
            ...
            {
                int i = 0;
                string test = "Wiazka " + doseload.beamNumber + Environment.NewLine + "Nazwa Dawka(Gy/cGy) SSD(mm) Gl(mm) Gl_rad(mm)" + Environment.NewLine;

                foreach (List<string> beamData in doseData)
                {
                    foreach (string data in beamData)
                    {
                        if ((i + 1) % 6 == 0 && i != 0)
                        {
                            test += data + Environment.NewLine;
                        }
                        else
                        {
                            test += data + " ";
